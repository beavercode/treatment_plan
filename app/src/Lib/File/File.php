...
class File
{
    ...
    public static function read($file, $length = -1, $mode = FILE_BINARY)
    {
        if (! is_file($file) && ! is_readable($file)) {
            throw new AppException('"' . $file . '" file not exists or not readable!');
        }
    ...
    public static function write($file, $data, $mode = 'a')
    {
        if (file_exists($file) && ! is_writable($file)) {
            throw new AppException('"' . $file . '" file not exists or not writable!');
        }
        if ($mode === 'w') {
    ...
    {
        if (! is_file($file) && ! is_readable($file)) {
            throw new AppException('Can\'t include file "' . $file . '""');
        }
    ...
    {
        if (! is_file($file)) {
            throw new AppException('"' . $file . '" is directory, can not remove');
        }

        if (! is_writable($file)) {
            throw new AppException('"' . $file . '" is not writable!');
        }
